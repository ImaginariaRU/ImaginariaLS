...
const TR_PARAM_COMBINATION = 18;
    public $tagsRules = array();
    public $entities1 = array('"' => '&quot;', "'" => '&#39;', '&' => '&amp;', '<' => '&lt;', '>' => '&gt;');
    public $entities2 = array('<' => '&lt;', '>' => '&gt;', '"' => '&quot;');
    public $textQuotes = array(array('«', '»'), array('„', '“'));
    public $dash = " — ";
        public $apostrof = "’"; // Автозамена
    public $dotes = "…";
    public $nl = "\r\n";
        public $defaultTagParamRules = array('href' => '#link', 'src' => '#image', 'width' => '#int', 'height' => '#int', 'text' => '#text', 'title' => '#text'); // <br/>, <img/>
        public $outBuffer = ''; // \n = <br/>
    public $errors;
    protected $text;
    protected $textBuf;
    protected $textLen = 0;
    protected $curPos;
    protected $curCh;    // Тег позволен
        protected $curChOrd;      // Параметр тега позволен (a->title, a->src, i->alt)
        protected $curChClass;     // Параметр тега влятся необходимым (a->href, img->src)
        protected $curParentTag;      // Тег может быть коротким (img, br)
        protected $states;        // Тег необходимо вырезать вместе с контентом (script, iframe)
        protected $quotesOpened = 0;      // Тег может содержать другие теги
        protected $brAdded = 0;      // Тег может содержать лишь указанные теги. В нём не может быть текста
        protected $state;     // Теги которые может содержать внутри себя другой тег
        protected $tagsStack;     // Тег в котором должен содержаться данный тег
        protected $openedTag;  // Преформатированные тег, в котором всё заменяется на HTML сущности типа <pre> сохраняя все отступы и пробелы
    protected $autoReplace;    // Auto add parameters + default values (a->rel[=nofollow])
        protected $linkProtocolAllow = array(); // Отключение типографирования для тега
        protected $linkProtocolAllowDefault = array('http', 'https', 'ftp');      // Не короткий тег с пустым содержанием имеет право существовать
    protected $isXHTMLMode = true;    // Тег в котором не нужна авто-расстановка <br>
    protected $isAutoBrMode = true;      // Тег обрабатывается callback-функцией - в обработку уходит только контент тега(короткие теги не обрабатываются)
        protected $isAutoLinkMode = true;    // Тег после которого не нужна автоподстановка доп. <br>
        protected $br = "<br/>";    // Тег обрабатывается callback-функцией - в обработку уходит весь тег
        protected $noTypoMode = false;    // Проверка на возможные комбинации значений параметров тега
    /**
