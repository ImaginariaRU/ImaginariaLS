...
class ModuleUser_EntityUser extends Entity
{
    ...
    {
        //@todo: отличается от LS 1.0.4 версии, правка для показа имени пользователя
        return $this->_getDataOne('user_profile_name') ?? $this->getLogin();

        /*
        $name = $this->_getDataOne('user_profile_name');
        if($name){
            return $name;
        }else{
