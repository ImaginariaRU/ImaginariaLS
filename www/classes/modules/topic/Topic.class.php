...
class ModuleTopic extends Module
{
    ...
    public function UploadTopicPhoto($aFile)
    {
        ...
        $sPath = Config::Get('path.uploads.images') . '/topic/' . date('Y/m/d') . '/';  // /www/uploads/images + /topic/ ....

        
        if (!is_dir(LIVESTREET_PATH_WWW . $sPath)) {
            if (!mkdir( $concurrentDirectory = LIVESTREET_PATH_WWW . $sPath, 0755, true ) && !is_dir( $concurrentDirectory )) {
                throw new \RuntimeException( sprintf( 'Directory "%s" was not created', $concurrentDirectory ) );
            }
        }

        $sFileTmp = LIVESTREET_PATH_WWW . $sPath . $sFileName;
        if (!move_uploaded_file($aFile['tmp_name'], $sFileTmp)) {
            return false;
        }
