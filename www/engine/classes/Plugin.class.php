...
abstract class Plugin extends LsObject
{
    ...
    static public function GetPath($sName)
    {
        ...
        return LIVESTREET_PATH_WWW . '/plugins/' . $sName . '/';
    }
    ...
    static public function GetTemplateWebPath($sName)
    {
        ...
        if (!isset(self::$aTemplateWebPath[$sName])) {
            $aPaths = glob(LIVESTREET_PATH_WWW . '/plugins/' . $sName . '/templates/skin/*', GLOB_ONLYDIR);
    ...
    static public function GetTemplatePath($sName)
    {
        ...
        if (!isset(self::$aTemplatePath[$sName])) {
            ...
            $sDir = LIVESTREET_PATH_WWW . "/plugins/{$sName}/templates/skin/{$sTemplateName}/";
    ...
    public function GetVersion()
    {
        ...
        $sPluginXML = LIVESTREET_PATH_WWW . '/plugins/' . strtolower($aMatches[1]) . '/' . ModulePlugin::PLUGIN_XML_FILE;
        if ($oXml = @simplexml_load_string( file_get_contents( $sPluginXML ) )) {
