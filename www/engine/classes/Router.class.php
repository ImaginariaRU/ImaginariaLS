...
class Router extends LsObject
{
     ...
     *
     * @param $aRequestUrl array реквеста
     * @return array
     */
    ...
    public function ExecAction($debug = false)
    {
        ...
        if (!preg_match('/^Plugin([\w]+)_Action([\w]+)$/i', $sActionClass, $aMatches)) {
            require_once(LIVESTREET_PATH_WWW . '/classes/actions/' . $sActionClass . '.class.php');
        } else {
            require_once(LIVESTREET_PATH_WWW . '/plugins/' . strtolower($aMatches[1]) . '/classes/actions/Action' . ucfirst($aMatches[2]) . '.class.php');
        }
        ...
        if ($debug) {
            $this->showSystemHooks();
        }
    }

    /**
     * @todo:
     * show system hooks as table and die.
     * this code MUST BE MOVED to /ADMIN section
     */
