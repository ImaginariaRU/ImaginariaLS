...
require_once LIVESTREET_PATH_WWW . '/engine/lib/internal/ProfilerSimple/Profiler.class.php';
...
class Engine extends LsObject
{
    ...
    {
        $sDirHooks = LIVESTREET_PATH_WWW . '/classes/hooks/';
    ...
    {
        if ($aPluginList = func_list_plugins()) {
            $sDirHooks = LIVESTREET_PATH_WWW . '/plugins/';
     ...
     * @param int $iFlag Маска по которой нужно вернуть результат. Доступные маски определены в константах CI_*
    ...
    public static function GetClassInfo($oObject, $iFlag = self::CI_DEFAULT, $bSingle = false)
    {
        ...
        if ($iFlag & self::CI_METHOD) {
            $sModuleName = $aResult[ self::CI_MODULE ] ?? self::GetClassInfo( $sClassName, self::CI_MODULE, true );
        ...
        if ($iFlag & self::CI_PPREFIX) {
            $sPluginName = $aResult[ self::CI_PLUGIN ] ?? self::GetClassInfo( $sClassName, self::CI_PLUGIN, true );
        ...
        if ($iFlag & self::CI_INHERITS) {
            $sInherit = $aResult[ self::CI_INHERIT ] ?? self::GetClassInfo( $sClassName, self::CI_INHERIT, true );
    ...
    public static function GetClassPath($oObject)
    {
        ...
        $sPath = LIVESTREET_PATH_WWW . DIRECTORY_SEPARATOR; //
