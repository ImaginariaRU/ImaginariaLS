...
function func_mkdir($sBasePath, $sNewDir)
{
    ...
    if (!is_dir($sDirToCheck)) {
        if (!mkdir( $sDirToCheck, 0755, true ) && !is_dir( $sDirToCheck )) {
            throw new \RuntimeException( sprintf( 'Directory "%s" was not created', $sDirToCheck ) );
        }
    }
}
...
function func_list_plugins($bAll = false)
{
    ...
    if ($bAll) {
        ...
        if ($aPluginRaw = @file($sPluginsListFile)) {
            ...
            $aPluginRaw = array_map(static function($item) {
                return trim($item);
            }, $aPluginRaw);
            
            $aPluginRaw = array_unique($aPluginRaw);
    
            $aPluginRaw = array_filter($aPluginRaw, static function($item) {
