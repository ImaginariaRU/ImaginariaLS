        ...
        if ($this->bUseStandartSession) {
            ...
            if (!session_id()) {
                ...
                if (@ini_get('session.use_only_cookies') === "0" and isset($aRequest[Config::Get('sys.session.name')]) and !is_string($aRequest[Config::Get('sys.session.name')])) {
                    //@todo: original code is: session_name($this->GenerateId());
                    die("Hacking attempt! Please check cookie PHP session name.");
                }
                /**
                ...
                if ($sUserAgent and
                    (
                        in_array($sUserAgent, $this->aFlashUserAgent) or
                        strpos($sUserAgent, "Adobe Flash Player") === 0
                    ) and
                    is_string(getRequest('SSID'))
                    and
                    preg_match("/^[\w\d]{5,40}$/", getRequest('SSID'))
                ) {
