...
require_once(LIVESTREET_PATH_ENGINE . '/lib/external/Smarty/libs/Smarty.class.php'); // 3.1.8
...
class ModuleViewer extends Module
{
    ...
    public function Init($bLocal = false)
    {
        ...
        if (!$bLocal) {
            ...
            if (file_exists($sFile = LIVESTREET_PATH_INSTALL . "/config/skins/{$skin}.config.php")) {
                Config::LoadFromFile($sFile, false);
            }
        }
        ...
        if (!is_dir($sCompilePath)) {
            if (!mkdir( $sCompilePath ) && !is_dir( $sCompilePath )) {
                throw new \RuntimeException( sprintf( 'Directory "%s" was not created', $sCompilePath ) );
            }
        }
    ...
    public function SmartyDefaultTemplateHandler($sType, $sName, &$sContent, &$iTimestamp, $oSmarty)
    {
        ...
        if (Config::Get('view.skin') != 'default') {
            ...
            if (preg_match('@^' . preg_quote(LIVESTREET_PATH_WWW) . '/plugins/([\w\-_]+)/templates/skin/' . preg_quote(Config::Get('view.skin')) . '/@i', $sName, $aMatch)) {
                $sFile = str_replace($aMatch[0], LIVESTREET_PATH_WWW . '/plugins/' . $aMatch[1] . '/templates/skin/default/', $sName);
            ...
            if (preg_match('@^([\w\-_]+)/templates/skin/' . preg_quote(Config::Get('view.skin')) . '/@i', $sName, $aMatch)) {
                $sFile = LIVESTREET_PATH_WWW . '/plugins/' . str_replace($aMatch[0], $aMatch[1] . '/templates/skin/default/', $sName);
                if ($this->TemplateExists($sFile)) {
                    return $sFile;
                }
            }
    ...
    protected function GetServerPath($sFile)
    {
         ...
         * Производим замену
         */
        $sFile = str_replace($sPathWeb, LIVESTREET_PATH_WWW, $sFile);
    ...
    {
        return str_replace(
            str_replace(DIRECTORY_SEPARATOR, '/', LIVESTREET_PATH_WWW),
