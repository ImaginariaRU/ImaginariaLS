    ...
    public function GetUserList(&$iCount, $iCurrPage, $iPerPage, $aFilter = Array(), $aSort = Array())
    {
        ...
        $sql =
            "SELECT {$sFieldList}
                ...
                LEFT JOIN {$table_session} AS us ON u.user_id=us.user_id 
                     AND us.session_key=(SELECT session_key FROM {$table_session} AS us1 
                                         WHERE us1.user_id=u.user_id 
                                         AND us1.session_date_last = (
                                            SELECT MAX(session_date_last) 
                                            FROM {$table_session} AS us2 
                                            WHERE us2.user_id=u.user_id LIMIT 1
                                            ) 
                                        LIMIT 1)
                WHERE {$sWhere}
                ORDER BY {$sOrder}
