...
<div id="wall-item-{$oWall->getId()}" class="js-wall-item comment-wrapper">
    ...
    {if $oUserCurrent}
        <form class="wall-submit wall-submit-reply" style="display: none">
            ...
            <button type="button" onclick="addReply(jQuery('#wall-reply-text-{$oWall->getId()}').val(), {$oWall->getId()}, {$oWallUser->getId()});" class="button button-primary js-button-wall-submit">{$aLang.wall_reply_submit}</button>
        </form>
    {/if}
</div>
{/foreach}
