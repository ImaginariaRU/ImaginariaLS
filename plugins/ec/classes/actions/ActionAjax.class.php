...
class PluginEc_ActionAjax extends ActionPlugin
{
	...
	protected function SaveComment()
	{
		...
		if (!($oComment = $this->Comment_GetCommentById(getRequest('commentId', null)))) {
			$this->Message_AddErrorSingle($this->Lang_Get('plugin.ec.ce_comment_error_not_found'), $this->Lang_Get('error'));
			return;
		}
		...
		if (!$this->oUserCurrent->isAdministrator()) {
			...
			if (Config::Get('plugin.ec.limit_edit_time') and strtotime($oComment->getDate()) < strtotime(date("Y-m-d H:i:s", time() - Config::Get('plugin.ec.limit_edit_time')))) {
				$this->Message_AddErrorSingle($this->Lang_Get('plugin.ec.not_access_time_limit'), $this->Lang_Get('error'));
				return;
			}
			if (Config::Get('plugin.ec.children_isset') and $this->PluginEc_Ec_GetCountChildrenByCommentId($oComment->getId())) {
				$this->Message_AddErrorSingle($this->Lang_Get('plugin.ec.not_access_children_isset'), $this->Lang_Get('error'));
				return;
			}
		}
		...
		$this->Viewer_AssignAjax('sTextEdit', $this->Lang_Get('plugin.ec.ce_comment_info_edit_user', array('login' => $this->oUserCurrent->getLogin(), 'date' => $oComment->getCommentDateEdit())));
		$this->Viewer_AssignAjax('sText', $sText);
		$this->Message_AddNotice($this->Lang_Get('plugin.ec.ce_comment_edit_ok'), $this->Lang_Get('attention'));
		return;
	}
