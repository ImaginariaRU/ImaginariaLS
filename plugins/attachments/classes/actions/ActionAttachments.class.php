...
class PluginAttachments_ActionAttachments extends ActionPlugin {
	...
	protected function CheckFile(){
		...
		if(!$_FILES['newfile']['error'] == 0) {
			...
			case UPLOAD_ERR_INI_SIZE: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_ini_size'),1,1); 
			case UPLOAD_ERR_FORM_SIZE: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_form_size'),1,1);
			case UPLOAD_ERR_PARTIAL: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_partial'),1,1);
			case UPLOAD_ERR_NO_FILE: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_no_file'),1,1);
			case UPLOAD_ERR_NO_TMP_DIR: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_no_tmp_dir'),1,1);
			case UPLOAD_ERR_CANT_WRITE: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_cant_write'),1,1);
			case UPLOAD_ERR_EXTENSION: $this->SetError($this->Lang_Get('plugin.attachments.upload_err_extension'),1,1);
