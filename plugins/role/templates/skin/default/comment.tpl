...
<section id="comment_id_{$oComment->getId()}" class="comment
	...
	{if !$oComment->getDelete() or $bOneComment or ($oUserCurrent and $oUserCurrent->isAdministrator())}
		...
		<div id="comment_content_id_{$oComment->getId()}" class="comment-content">
			<div class=" text">
				{if $oComment->getUserId()!=45 or $oUserCurrent}
					{$oComment->getText()}
				{else}
					Залогиньтесь, чтобы увидеть содержимое.
				{/if}
			</div>
		</div>
        ...
        <div id="info_edit_{$oComment->getId()}" class="info_edit" {if !$oComment->getCommentEditUserId()} style="display: none;"{/if}>
            ...
            {$aLang.plugin.role.comment_info_edit_user|ls_lang:"login%%`$oUserEdit->getLogin()`":"date%%`$oComment->getCommentDateEdit()`"}
	    {/if}
        </div>
		...
		<ul class="comment-info">
			<li class="comment-author">
				...
				<a href="{$oUser->getUserWebPath()}">{if $oUser->getProfileName()!=""}{$oUser->getProfileName()}{else}{$oUser->getLogin()}{/if}</a>
			</li>
